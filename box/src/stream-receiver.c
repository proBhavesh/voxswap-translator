                            ...
                            const uint8_t *payload, size_t payload_len)
{
    ...
    /* Parse optional target language fields from REGISTER payload.
     * New format: speakerName\0sourceLanguage\0targetLang1\0targetLang2\0
     * If the phone sends old format (no target fields), this is safely skipped. */
    size_t src_lang_end = (size_t)(lang_end - payload) + 1;
    const char *target_lang1 = NULL;
    const char *target_lang2 = NULL;

    if (src_lang_end < payload_len) {
        const uint8_t *tgt1_end = memchr(payload + src_lang_end, '\0',
                                          payload_len - src_lang_end);
        if (tgt1_end) {
            target_lang1 = (const char *)(payload + src_lang_end);
            size_t tgt2_offset = (size_t)(tgt1_end - payload) + 1;
            if (tgt2_offset < payload_len) {
                const uint8_t *tgt2_end = memchr(payload + tgt2_offset, '\0',
                                                  payload_len - tgt2_offset);
                if (tgt2_end) {
                    target_lang2 = (const char *)(payload + tgt2_offset);
                }
            }
        }
    }

    /* If admin provided at least one target language and box has none yet, apply them */
    if (target_lang1 && target_lang1[0] != '\0') {
        char cur1[LANG_CODE_MAX], cur2[LANG_CODE_MAX];
        settings_get_languages(sr->settings, cur1, cur2);
        if (cur1[0] == '\0' && cur2[0] == '\0') {
            const char *tgt2 = (target_lang2 && target_lang2[0] != '\0')
                               ? target_lang2 : "";
            settings_set_languages(sr->settings, (uint8_t)slot,
                                    target_lang1, tgt2, true);
            printf("stream-receiver: speaker %d set initial target languages: %s, %s\n",
                   slot, target_lang1, tgt2[0] ? tgt2 : "(none)");
        }
    }
                                 ...
                                 const uint8_t *payload, size_t payload_len)
{
    ...
    int result = settings_set_languages(sr->settings, spk->speaker_id, lang1, lang2, false);
