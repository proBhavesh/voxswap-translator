 ...
 * is dispatched to a per-stream-type executor. Each stream type (original, target1, target2)
 * has its own single-threaded executor so all 3 streams send in parallel while maintaining
 * packet ordering within each stream. Pre-allocated buffers per stream avoid GC pressure.
 */
public class AudioStreamer {
    ...
    /* Per-stream counters — each index only accessed by its own executor, no sync needed */
    private final int[] sequenceCounters = new int[NUM_STREAM_TYPES];
    private final int[] sampleOffsets = new int[NUM_STREAM_TYPES];

    /* Pre-allocated per-stream buffers — each index only accessed by its own executor */
    private final byte[][] chunkBuffers = new byte[NUM_STREAM_TYPES][BYTES_PER_PACKET];
    private final byte[][] packetBuffers = new byte[NUM_STREAM_TYPES][PACKET_SIZE];

    /* WiFi network binding — set by WalkieTalkieService when WiFi is acquired */
    private volatile Network wifiNetwork;

    /* Threading — one executor per stream type for parallel sending */
    private final ExecutorService[] streamExecutors = new ExecutorService[NUM_STREAM_TYPES];
    ...
    public void start(String host, int speakerId) {
        ...
        try {
            ...
            /* Bind to WiFi network — ensures UDP packets go through WiFi even when
             * Android prefers mobile data (no internet on WiFi). */
            Network net = wifiNetwork;
            if (net != null) {
                try {
                    net.bindSocket(udpSocket);
                    Log.d(TAG, "Socket bound to WiFi network");
                } catch (IOException e) {
                    Log.w(TAG, "Failed to bind socket to WiFi network: " + e.getMessage());
                }
            }

            for (int i = 0; i < NUM_STREAM_TYPES; i++) {
                streamExecutors[i] = Executors.newSingleThreadExecutor();
            }
            isActive = true;
    ...
    public void stop() {
        ...
        for (int i = 0; i < NUM_STREAM_TYPES; i++) {
            if (streamExecutors[i] != null) {
                streamExecutors[i].shutdownNow();
                streamExecutors[i] = null;
            }
        }
    ...
    /**
     * Set the WiFi Network handle for per-socket binding. When non-null, the UDP
     * socket is bound to this network so traffic goes through WiFi instead of
     * mobile data. Called from WalkieTalkieService when WiFi network is acquired/lost.
     */
     ...
     * Dispatched to the per-stream executor so different stream types send in parallel
     * while packets within each stream remain ordered.
     */
    private void streamAudio(byte[] pcm16kHz, int streamType) {
        if (!isActive || streamType < 0 || streamType >= NUM_STREAM_TYPES) return;

        streamExecutors[streamType].execute(() -> {
            byte[] chunk = chunkBuffers[streamType];
            byte[] packet = packetBuffers[streamType];
            int offset = 0;
            while (offset < pcm16kHz.length && isActive) {
                int chunkSize = Math.min(BYTES_PER_PACKET, pcm16kHz.length - offset);

                /* Copy PCM data; zero-pad the tail only when the last chunk is shorter */
                System.arraycopy(pcm16kHz, offset, chunk, 0, chunkSize);
                if (chunkSize < BYTES_PER_PACKET) {
                    Arrays.fill(chunk, chunkSize, BYTES_PER_PACKET, (byte) 0);
                }

                buildPacketInto(packet, streamType, sequenceCounters[streamType], sampleOffsets[streamType], chunk);
                sendPacket(packet);
    ...
    /**
     * Write a 650-byte UDP packet into the provided buffer (big-endian header + LE PCM payload).
     * Uses a pre-allocated buffer to avoid per-packet allocation.
     */
    private void buildPacketInto(byte[] packet, int streamType, int seq, int timestamp, byte[] pcmChunk) {
